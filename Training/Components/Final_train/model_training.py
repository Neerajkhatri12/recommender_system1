...
class ModelTraining:
    ...
    def train_final_model(self,loop_count, X_train, y_train):
        try:
            ...
            # Initialize a new NMF with the best hyperparameters
            hyperparams_file = f'best_params_nmf_{loop_count}.json'
            hyperparams_directory = self.config.best_model_params_nmf
    ...
    def filter_hyperparams(params):
      # Extract only the parameters related to the classifier (NMF)
      nmf_hyperparams = {key.replace('classifier__', ''): value for key, value in params.items() if key.startswith('classifier__')}
      return nmf_hyperparams
